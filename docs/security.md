...
Structured browser automation is executor-owned and requires the separate `browser:use` OAuth scope plus the executor-advertised `browser.v1` capability. Browser contexts are fresh and ephemeral rather than attached to a user's normal browser profile, and session teardown closes every browser resource owned by that session before confirming executor-side absence. Navigation accepts only HTTP(S) and `about:blank`, so the browser surface cannot use `file:` URLs as a path-policy bypass. Screenshots may be written only through the owning session's workspace path resolver, never overwrite an existing path, and remain subject to image-byte plus full-page dimension/pixel limits. Snapshot text and element metadata are bounded; current form-control values are not surfaced as element text. Audit records keep browser/session/page identifiers and coarse action shape, but omit page body/element text/page titles and backend error diagnostics, reduce URLs to origins, hide non-ref selectors, and redact entered/select/press/wait-for-text values. Arbitrary Playwright script execution and persisted profile/storage-state import are deliberately outside this initial surface.
