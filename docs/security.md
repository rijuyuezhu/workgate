...
- Private values, OAuth tokens, dynamic client information, absolute expiry timestamps, and discovered authorization-server metadata are stored under `state_dir/agent_auth`. The store is versioned and size-bounded, uses a cross-process lock and atomic replacement, and is owner-only (`0700` directory and `0600` files on POSIX). Credential-bearing manifest entries require a stable `integrationId`; this identity, not the mutable `mcpServers` key, scopes private credentials and the bounded retired-value redaction history. Keep the same `integrationId` when renaming a server. A history gap or eviction fails closed only for that integration, not for unrelated MCP servers. Rewriting identical token, secret, or client-secret material does not consume a new redaction-history revision.
- Literal `env` / `headers` remain supported for compatibility. Current literal values are conservatively redacted from that server's own public metadata/results/errors. Only literals whose key is credential-like (for example `Authorization`, token, secret, password, cookie, or API-key fields) are copied into durable retired-value history, and therefore require a stable `integrationId`. Ordinary literals such as `MODE=production` or `X-Mode: 1` are not persisted as cross-operation secret history. Credentials under arbitrary application-specific keys should use structured secret references instead of literal values. Corrupt or unsupported private state fails explicitly and is not reset automatically.
...
The bearer token and bearer URL are returned only by `create_file_link`; current-version writes never persist the plaintext bearer. Durable link metadata stores a non-secret `link_id`, the token SHA-256 and short fingerprint, the immutable `payload_id`, expiry/counter metadata, and file metadata. `list_file_links` exposes only the management identity/fingerprint and metadata, so it cannot reconstruct a lost bearer URL; `revoke_file_link` revokes by `link_id`. Incoming `/download/{token}` requests are matched against the stored SHA-256 digest.

Immutable payload bytes are stored privately under feature-owned subdirectories of the XDG data namespace, for example `data_dir/control/payloads/download`; primary/backup link metadata and the cross-process lock remain under `state_dir`. Payload size and SHA-256 are checked before serving. Each feature cleans only its own staging and unreferenced committed payloads, so download cleanup cannot delete transfer payloads. Expired, revoked, exhausted, malformed, and stale interrupted-transfer resources are removed by the owning feature. Current-version metadata is structurally validated before it can drive payload cleanup. A corrupt primary store is recovered from its backup; if both copies are invalid, the service refuses to silently reset current-version link state. When a healthy current primary is loaded, any existing recovery copy must be the same fully validated v3 state; legacy, malformed, unsupported, or stale backups are scrubbed before service continues. This architecture migration is intentionally breaking: on first access to the file-link store after upgrade, legacy version-1/version-2 registries that persisted plaintext bearer tokens are invalidated and their old snapshot artifacts are pruned rather than carrying those bearer secrets forward.
...
Control transfer metadata and spools live under `state_dir/remote_transfers` with private permissions, active-count and aggregate-byte quotas, expiry cleanup, and no symlink following. Cross-executor copies stage through that control spool instead of giving one executor network authority over the other. A restarted control process can discard a stale process claim and resume validated state; each successful authenticated request extends only the current short-lived grant.
...
Tracked shell and control-managed jobs persist metadata under `state_dir` with owner-private files and atomic primary/backup replacement. Job-store thread and file locks share a bounded acquisition budget; public contention errors do not reveal the private lock path, while local audit records retain enough detail for diagnosis.
