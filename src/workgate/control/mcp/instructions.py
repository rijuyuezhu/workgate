...
- Start substantial workspace work with `session_start(workdir=...)` and pass the returned shared `session_id` to every machine-facing tool. The workdir is required; ask when unclear, otherwise infer it from the task, repository, or paths. Omit `executor_id` only when exactly one eligible executor is online; otherwise select the intended executor explicitly. Use `session_change_cwd` when the working directory changes. Follow the semantic agent workflow: `read` for file/directory context, `search(pattern, paths=...)` for content discovery, `hashline_edit` as the default edit tool for existing files, `write_file` for new files or intentional whole-file replacement, and `bash` for terminal work. Use `edit_lines` only when exact structured path/start/end/replacement data is already available. Use `apply_patch` only when the caller already has a portable unified diff or `*** Begin Patch` compatibility envelope; keep `hashline_edit` as the default grounded edit path. `workspace_search` and `fetch` retain connector-compatible response shapes but are executor-backed and require the same `session_id`.
- For substantial multi-step work, use `read_session_task(session_id)` as the durable handoff, `report_session_progress` for objective/summary/findings/next-action/blockers and semantic task status, and `update_session_plan` for stable plan steps. Mutations require the latest `expected_revision`; reload after a conflict instead of overwriting newer state. Task state is control-owned and tied to the explicit Workgate session: executor disconnect/restart does not erase it, and `session_end` leaves it readable as history while preventing further mutations. `read_todos`/`write_todos` are compatibility views over the same plan steps, not a second checklist.
...
# Shell and Executors
- Prefer `bash` over legacy shell/job/session tools. By default it runs bounded non-interactive commands on the executor bound to the shared session. Use `async_=true` for tracked long-running non-interactive work that produces a `job_id` in the same agent session. Use `pty=true` for executor-side interactive shells, REPLs, servers, or commands that need later input; PTY mode returns a `shell_id`, not a `job_id`, and the `shell_id` is separate from the agent `session_id`.
...
- New machines pair through `workgate executor connect <control-url>`. Start normal machine work with `session_start(workdir=..., executor_id=...)`; omit `executor_id` only when exactly one eligible executor is online. File, search, edit, shell/job, PTY, `session_copy`, skill, and file-link operations route through the executor bound to that shared `session_id`. Durable task/progress/plan state and the Todo compatibility view are control-owned but still require that same explicit `session_id`; they never rebind or infer a different execution context.
